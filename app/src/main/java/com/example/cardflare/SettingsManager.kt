...
fun updateSetting(settingName: String, newState: Any) {
    AppSettings.value = AppSettings.value.toMutableMap().apply {
        val setting = this[settingName]
        if (setting != null) {
            // Modify the state of the setting
            this[settingName] = setting.copy(state = newState)
        }
    }
}
public var AppSettings = mutableStateOf( mapOf(
